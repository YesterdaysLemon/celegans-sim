...
| **Sensory pathways** `worm/senses.py` | **cell identity and polarity only** — ASEL/ASER as an ON/OFF pair, AWC as an OFF cell beside AWA, ASH/ADL/ASK for the repellent at the nose and PHA/PHB for the same repellent at the tail (Hilliard 2002), AFD warm, URX/AQR/PQR for the oxygen level and upshift and BAG for the downshift (Zimmer 2009), ALM/AVM and PLM for touch, OLQ/FLP/CEP for nose touch | adapting differential transduction (with documented tonic exceptions); the proprioceptive receptive-field construction | every gain and adaptation time constant |
...
| **Sleep** `worm/sleep.py` | RIS's identity, its GABAergic wiring onto AVE/RIB/RIM and the head motors, FLP-11 as its peptide (Turek 2013, 2016); satiety quiescence as the adult sleep (You 2008) | the satiety homeostat, the Schmitt-triggered bout, arousal clearing the standing peptide | `ris_drive`, the thresholds, `tau_sleep` — all on a deliberately compressed clock |
