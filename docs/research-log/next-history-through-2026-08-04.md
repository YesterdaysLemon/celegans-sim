...
that state is **240,304 bytes, 235 kB**, measured off the allocator's own per-worm stride
...
- ~~Multi-worm. The engine is one `Simulation` object; nothing prevents several.~~
  **Done with the WebAssembly port.** The connectome is read-only and shared, so a second
  animal duplicates only state -- 240,304 bytes of it, 235 kB, measured; the viewer runs n
