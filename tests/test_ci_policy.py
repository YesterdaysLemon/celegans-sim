...
NO_CI_NEEDED = {
    ...
    ".github/workflows/scorecard.yml":
        "a dispatch-only maintenance tool (re-measure the scorecard and commit the "
        "emitted pair). Dispatch-only for the same reason the consistency test does not "
        "gate freshness: measuring on every push would turn every legitimate "
        "behavioural change into a mandatory repin.",
    "docs/scorecard.md":
        "generated by tools/scorecard.py --emit, and its consistency with README.md IS "
        "gated -- tests/test_scorecard_doc.py, under tests/**, fails whenever the two "
        "copies disagree. An edit to this file alone schedules nothing, and the next "
        "commit that runs the python suite catches the drift.",
    "README.md": "prose",
