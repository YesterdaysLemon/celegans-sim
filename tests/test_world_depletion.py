...
def test_an_animal_that_grazes_dims_its_own_lawn(params):
    ...
    One worm on a 1.2 mm lawn for 30 s. It takes 0.058956 units off the plate -- the lawn
    holds about 8.8, so f_p ends at 0.997888 -- and that has to show up in both fields.
    ...
    assert eaten == pytest.approx(0.058956, abs=1e-6)
    assert w._patch_frac[0] == pytest.approx(0.997888, abs=1e-6)

    # Oxygen: the deficit is shallower than it was, so the animal is standing in more
    # oxygen than a full lawn would leave it.
    assert float((before_o2 - w.o2_deficit).max()) == pytest.approx(3.168e-04, rel=5e-3)
